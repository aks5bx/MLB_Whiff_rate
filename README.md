...
In order to understand the relationship between these three importance features and whiff_prob, I turned to my test data that I had withheld from model training. I took the test data and artificially changed these three features. After changing these features I found the new predictions for whiff_prob and compared the new predictions to the original predictions I had generated using the unchanged test data. Based on this change, I was able to get a rough quantification of the impact of these variables. My findings for changing individual variables are below: 
...
These are reasonably helpful changes. However, I decided to try to change all of the variables instead of changing them one at a time. The results were even more pronounced: 
- Shifting Plate Side to the left AND Shifting Plate Height down AND Changing Pitch Type to a Curveball or Slider increased predicted whiff_prob by a whopping **7%** from 11% to 18% - a major improvement. 

As a sanity check, I verified that these findings held true no matter what the other variables were (I especially checked for batter_side). 

## Conclusion 

The results I found show that in a vacuum, changes to where the ball is located (both horizontal and vertical location) and changes to the type of pitch that is thrown can drastically impove predicted whiff probability (and consequently whiff rate). Of course, baseball is not played in a vacuum. Many other factors are at play including the batter's personal tendencies, pitch variation, and hundreds of other components. 

However, the findings I produced are statistically significant and at the very least indicate that a non-trivial improvement can be generated by tweaks in a pitchers pitch delivery. As such, I recommend targeting the bottom left-hand corner of the strike zone as one (of many) method to increase whiff probability. 
